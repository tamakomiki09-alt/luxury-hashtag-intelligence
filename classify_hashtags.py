...
CODEBOOK = {
    "Property identity": (
        "THIS property's own name or account handle, in any script or "
        "abbreviation, including when a modifier is attached. The bare "
        "identifier, not a slogan. The name must identify the Tokyo property "
        "specifically. Examples: amantokyo, アマン東京, ザキャピトルホテル東急, "
        "ritzcarltontokyo, rctokyo, capitolhoteltokyo, リッツカールトン東京, "
        "luxurycapitolhotel, tokyuhotelexperience, リッツ婚. "
        "DOES NOT INCLUDE the bare chain or group name with no city attached "
        "(parkhyatt, ritzcarlton, パークハイアット), or a sister property in "
        "another city (rcistanbul, アマン京都, アマネム) — those point outward to "
        "the group and belong in Group and loyalty."
    ),
    "Group and loyalty": (
        "The parent company, a sister property in another city, a collection "
        "the hotel belongs to, or a loyalty or booking programme. Points "
        "outward to an affiliation rather than to this property. Includes a "
        "slogan owned by a consortium rather than by this house. Examples: "
        "amanresorts, worldofhyatt, thepreferredlife, ipreferrewards, "
        "believeintravel, marriottbonvoy, parkhyatt, ritzcarlton, rcistanbul, "
        "アマン京都, アマネム, espa."
    ),
    "Brand philosophy": (
        "A coined, proprietary phrase expressing what THIS property's own "
        "operating brand stands for — a campaign line or positioning "
        "statement, not a name and not a description of a thing. Examples: "
        "luxuryispersonal, rcmemories, welcometojanu, ジャヌへようこそ, "
        "魂を呼び覚ます, rekindlethesoul, thespiritofaman, capitolmoments, "
        "penmoments. DOES NOT INCLUDE a slogan belonging to a consortium or "
        "booking programme rather than to this house (believeintravel, "
        "thepreferredlife) — those are Group and loyalty."
    ),
    "Named outlet and talent": (
        "A PROPER NOUN naming a specific restaurant, bar, lounge, patisserie, "
        "spa or shop inside the hotel, or a named chef, designer or artist "
        "associated with it. The test is whether the tag contains a name, not "
        "whether it denotes a physical room. Examples: theloungebyaman, "
        "arvabyaman, ザカフェbyアマン, アマンスパ, amanspa, 武蔵byアマン, kozue, "
        "newyorkbar, clubonthepark, 星ヶ岡, 水簾, keikobayashi, 小林圭, "
        "ケリーヒル, nomataminoru. Brand-prefixed programme names count as "
        "proper nouns: amanwellness, amanfoodie, amandesign, amanadventures, "
        "amanessentials, アマンエッセンシャルズ. "
        "DOES NOT INCLUDE generic facility types with no name attached "
        "(library, lobbylounge, ロビーラウンジ, ホテルエントランス, "
        "ペストリーブティック, europeanbrasserie, hotelart), or individual menu "
        "items, dishes and drinks (tempuradelight, strawberryfrozenmojito, "
        "rokuジン, クッキーアソート, amantokyoafternoontea) — those are Occasion "
        "and service."
    ),
    "Occasion and service": (
        "A generic service, facility, dining occasion, room feature, or a food "
        "or drink item, named as a TYPE rather than by a proper name. The "
        "default for anything the hotel serves or provides that carries no "
        "name of its own. Examples: afternoontea, アフタヌーンティー, ヌン活, "
        "ホテルディナー, ホテルランチ, ホテル朝食, ホテルステイ, clublounge, "
        "ロビーラウンジ, ホテルエントランス, library, ウェルネス, スパ, ヨガ, "
        "朝食, パフェ, スイートルーム, ルームサービス, viewfromguestroom, "
        "ウエディングドレス, luxurydinner, エクスペリエンス. "
        "Also covers individual ingredients, dishes and drinks, whether or not "
        "they are Japanese: 苺, ズワイ蟹, 鰻, sushi, kaiseki, ricecake, 北京ダック, "
        "caviar, ウィスキー, shinemuscat, ブルーベリー, フランス料理, "
        "ガストロノミー, europeanbrasserie, tempuradelight, strawberrymartini. "
        "A promotional suffix (delight, delights, lovers, heaven, experience) "
        "does not change the category — strip it and code what remains."
    ),
    "Japanese cultural register": (
        "Invokes Japanese cultural PRACTICE or CRAFT as a tradition — "
        "signalling authenticity or rootedness in Japan. Reserved for "
        "tradition-level terms, not for individual foods. Examples: omotenashi, "
        "おもてなし, 日本のおもてなし, ikebana, 生け花, 草月, 草月流, "
        "japaneseflowerarrangement, 日本の伝統, washoku, edokiriko, 江戸切子, "
        "bonsai, 扇子, 人力車, goshuincho. "
        "DOES NOT INCLUDE individual dishes, ingredients or cuisine types "
        "(sushi, kaiseki, 鰻, 苺, ricecake, japanesecuisine), which are Occasion "
        "and service; named cultural sites and museums (日枝神社, momat, "
        "根津美術館), which are Location; or calendar observances (tanabata, "
        "setsubun, 桜, 紅葉狩り, 年末のご挨拶), which are Season and limited "
        "time. Non-Japanese cuisine (フランス料理, europeanbrasserie) never "
        "belongs here."
    ),
    "Season and limited time": (
        "Tied to a season, holiday, festival, anniversary, dated external "
        "event or explicitly time-bound offer. If you could guess roughly when "
        "the post went up from the tag alone, it belongs here. Examples: "
        "クリスマス, christmas, 桜, sakura, 桜の名所, 紅葉狩り, 春, 秋, 初夏, "
        "期間限定, 限定, tanabata, setsubun, 立冬の日, 福まめ, 父の日, 母の日ギフト, "
        "バレンタインデー, 年末のご挨拶, pht30周年, ジャヌ東京1周年, 2023年, "
        "japancakeshow, 山口県フェア, accelerateaction, reopening. "
        "Recurring slots that are not calendar dates (平日ランチ, 週末ご褒美) are "
        "Occasion and service."
    ),
    "Location": (
        "A NAMED geographic place: country, city, ward, neighbourhood, "
        "district, landmark, transit hub or cultural institution. The tag must "
        "name somewhere you could go. Examples: 赤坂, 溜池山王, tokyo, japan, "
        "marunouchi, ginza, 東京ミッドタウン, 大手町の森, アークヒルズ, 日枝神社, "
        "momat, 東京国立近代美術館, 根津美術館, imperialpalace, 小樽, "
        "カーネギーホール. "
        "DOES NOT INCLUDE descriptive terms for views or travel with no place "
        "named (景色, tokyocityscape, japantrip, 東京夜景), which are Discovery "
        "stack; nor place-plus-generic-noun compounds (東京ホテル, 東京グルメ), "
        "also Discovery stack."
    ),
    "Discovery stack": (
        "A broad, generic search term aimed at reach, with no brand, outlet or "
        "specific place named. Three shapes qualify: a place name bolted to a "
        "generic noun (東京ホテル, 東京グルメ, 東京スイーツ, tokyocityscape, "
        "breakfastintokyo, tokyoricecakes, japantrip, visitjapan); a category "
        "or tier claim about the property itself (ラグジュアリーホテル, 5つ星ホテル, "
        "絶景ホテル, 都心のオアシス, ミシュラン東京, michelinkey, tatlerbest2025); "
        "and platform-community or unowned lifestyle tags (flowerstagram, "
        "travelgram, アートのある暮らし, tokyoluxurylife, hotellife). "
        "DOES NOT INCLUDE a hotel service with no place name attached "
        "(ホテル朝食, ホテルステイ, luxurydinner), which is Occasion and service."
    ),
    "Unclassifiable": (
        "Emoji only, a bare subject noun with no discernible function "
        "(百合, flower, waterelm), an unrelated personal handle, gibberish, "
        "caption text captured as a tag, or a fragment whose meaning cannot be "
        "determined without seeing the post. Use sparingly. A recognisable food "
        "or drink is NOT unclassifiable — it is Occasion and service."
    ),
}
